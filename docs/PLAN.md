...
**Replaced with:** on the synthetic track, directional predictability is ≈ 0 *by construction* (20-day blocks destroy autocorrelation beyond that horizon; single-name daily return autocorrelation is ≈ 0 in the source data anyway). But **volatility clustering survives the block bootstrap**, so *risk* is predictable even where *direction* is not. Managing exposure against forecastable volatility is therefore a genuine ex-ante edge — and it is exactly the edge a Sharpe-based metric rewards. The masked-real track retains real directional structure; the gap between the tracks *is* the leakage measurement (D9). Friction is fixed at 10 bps/side and described as **realistic**, never as *calibrated*. Baseline net performance per track is reported empirically, not asserted in advance.
